...
import { useSlider } from '@telegram-apps/telegram-ui/dist/components/Form/Slider/hooks/useSlider';
import {
  Touch,
  type TouchEvent
} from '@telegram-apps/telegram-ui/dist/components/Service/Touch/Touch';
...
export const Slider = ({ className, disabled, before, after, ...restProps }: SliderProps) => {
  ...
  return (
    <div className={classNames(styles.wrapper, disabled && styles['wrapper--disabled'], className)}>
      ...
      <Touch
        ...
        <div className={styles.track}>{steps !== undefined && <SliderSteps steps={steps} />}</div>
        <div
