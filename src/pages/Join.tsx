...
export function Join() {
    ...
    const fetchCurrentUser = async () => {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/referrals/currentUser`, { headers: { tmaInitData: (window as any).Telegram.WebApp.initData } })
    ...
    const getUserName = () => {
        return String(currentUser.username || currentUser.first_name || currentUser.last_name || currentUser.id);
    }
    ...
    return (
        <>  
            ...
            <Paper sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer'}}>
                <Typography textAlign={'center'}>Hello {getUserName()}!</Typography>
                <Typography textAlign={'center'}>User {linkOwner.name} would like to invite you to the community "{linkOwner.title}".</Typography>
                <Button fullWidth onClick={handleJoin}>Join the community</Button>
            </Paper>
        </>
    )
}